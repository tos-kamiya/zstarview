...
terrain-clip, common inverse-projection, and molecular-cloud helper boundaries
now accept `ViewProjection` rather than separate view center/FOV values. The
remaining projection helpers still need an audit to distinguish useful
grouping from APIs that intentionally accept only a subset of projection data.
...
6. **Sec. 4 in progress:** Continue the `ViewProjection` consumption audit;
   cloud overlay, dim-alt ring, terrain-clip, inverse-projection, and
   molecular-cloud helpers are migrated.
