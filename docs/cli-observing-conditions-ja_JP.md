...
| `--edge-fov-deg DEGREES` | ウィンドウ端に対応する投影スケールを指定します。`90` なら、ウィンドウ端は視線中心から `90°` に対応します。 | `90` |
...
| `--height-add-m METERS` | 観測基準の上に追加する高さをメートルで指定します。既定の追加高さ `1.7` を置き換えます。この `1.7m` は、立っている観測者の目線高さの目安です。タワーや山のビューポイント自体の高さ・標高とは別に扱われます。 | `1.7` |
| `-B`, `--use-building-top` | 解決地点の約 5m 以内に建物が見つかった場合、その建物の最も高い頂部を観測基準として使います。 | off |
...
ウィンドウのサイズ変更中も、視点操作時と同じ簡略表示モードを使い、応答性を保ちます。
