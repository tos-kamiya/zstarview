...
- キーボード視線変更時の簡易表示モード
  - 方向キー操作中は、後にリサイズとも共有する `viewport_interaction_mode` で `render_view_center` を即時更新し、全レイヤー再計算ではなく再投影ベースで追随するようにした。
  - 最後の方向キー入力から 1 秒間は、`vmag <= 4.0` の明るい星とガイドライン、方位ラベル、天頂マーカーのみを表示する。
  - この簡易モード中は惑星、全星等の星空、sky disc、雲、地形地平線、DSO、アステリズムを表示しない。
