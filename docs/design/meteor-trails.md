...
- `render/meteors.py`
  - 発生時刻に保存したAlt/Az端点を画面座標へ投影して、始点から終点の80%で太くなる
    ひし形の発光軌跡として描画
  - 淡い黄色の外側形状と白い内側形状を重ね、観測時刻ラベルはその中間色で描画
...
ステータス用には、表示軌跡がある場合はその`beginning_utc`の最古・最新を、0件の場合は
`window_start`・`window_end`を表示時刻との差へ変換する。古い側は切り上げ、新しい側は
切り捨て、`M {count}, {oldest}-{newest}h ago`としてASCIIだけで組み立てる。これにより
表示件数制限で除外された古い軌跡を、表示中の範囲に含めない。

表示する軌跡は、選択窓内の`Beginning UTC Time`が新しい順に既定で最大200件とする。指定値を
超える場合は古い軌跡から表示対象外とする。窓の時間範囲は表示対象を制限する前の選択窓に基づき、
...
投影後の始点・終点を結ぶ方向に対して法線方向の幅を与え、始点と終点を細く、
始点から80%の位置を最大幅とする四頂点のポリゴンを作る。外側の幅は始点・終点
`0.4 px`、最大幅`1.25 px`、内側はそれぞれ`0.18 px`、`0.72 px`とする。
外側は`(255, 220, 120)`、内側は`(255, 255, 255)`、ラベルは`(255, 238, 188)`を使う。
