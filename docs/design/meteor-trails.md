...
- `render/meteors.py`
  - 発生時刻に保存したAlt/Az端点を画面座標へ投影して、始点から終点の80%で太くなる
    ひし形の発光軌跡として描画
  - 本体を白またはAtlasでは濃いグレーで描画し、観測時刻ラベルも本体と同色で描画
...
始点から80%の位置を最大幅とする四頂点のポリゴンを作る。本体のみを描画し、
始点・終点の半幅を`0.8 px`、中央最大の半幅を`1.75 px`とする。
通常版の本体とラベルは`(255, 255, 255)`、Atlasの本体とラベルは`(32, 32, 32)`を使う。
