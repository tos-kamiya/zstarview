...
- `meteors/projection.py`
  - 1000km候補抽出
  - WGS84地理座標からECEFへの変換
  - 1000km候補の各端点を発生時刻の観測地点基準Alt/Azへ変換
- `meteors/service.py`
...
- `render/meteors.py`
  ...
  - 選択窓の終端から18時間までは一定alpha、窓の先頭までの6時間は線形減衰
...
フェードの年齢基準には`display_time`ではなく`window_end`を使う。これにより、公開遅延が
そのまま全軌跡の透明化につながることを防ぐ。各端点の位置は各観測の実際の開始UTC時刻に
観測地点から見えたAlt/Azとして保持し、表示時刻が変わっても再投影しない。
...
端点が観測時刻の水平線下にある場合も、1000km以内の候補であれば除外しない。
...
しない。画面座標、色、不透明度、窓終端基準の18時間固定＋6時間線形フェードはこの段階の
モデルへ含めない。
