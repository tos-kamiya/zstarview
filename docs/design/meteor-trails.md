...
- `render/meteors.py`
  ...
  - 表示時刻からの経過時間で線形減衰し、72時間で30%、それ以降は30%
...
フェードの年齢基準には`window_end`ではなく`display_time`を使う。不透明度は経過0時間で
1.0、72時間で0.3となる線形減衰とし、それ以降は`0.3`のままとする。公開遅延のある
古い窓ほど薄くなるが、表示対象の軌跡は消さない。
