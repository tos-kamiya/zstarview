...
街灯系 glow は、現行の帯ポリゴンから離れて、夜間光タイルを使う離散レイサンプリングへ寄せる試作を前提にしてよい。レイ上の等間隔サンプルで夜間光タイルのピクセル値を拾い、そのピクセルをそのまま底面積の柱として扱う。柱は横方向へ広げず、固定高さへ押し出して高さ方向だけ減衰させる。試作段階では、サンプル間隔は 1km、加算前の強度は log 圧縮としてよい。最終的な描画は、`az` ごとに近い距離からレイを進め、手前の稜線や地平線より下の部分を切り落としながら可視部分だけを積算する。
...
3. 低解像度キャンバスに、night-light glow と ridge glow を同じ面へ描く。
   - 街灯系 glow は、レイ上サンプルで拾った夜間光ピクセルを柱として積み上げる。
   - 柱はタイルピクセルと同じ底面積を持つものとして扱い、横方向へは広げない。
   - 高さは固定し、柱内では高さ方向にだけ減衰を入れる。
...
街灯系 glow の試作では、密度場は「レイ上のサンプル点ごとの夜間光ピクセル値の累積」として扱ってよい。補間や横方向の拡散は後回しにして、まずはざっくりした見た目の改善を優先する。必要な連続性は、`az` ごとの距離レイを 1 km 刻みで積算し、可視な部分だけを残すことで確保してよい。
