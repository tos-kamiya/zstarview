...
街灯系 glow は、現行の帯ポリゴンから離れて、夜間光タイルを使う離散レイサンプリングへ寄せる試作を前提にしてよい。レイ上の等間隔サンプルで夜間光タイルのピクセル値を拾い、そのピクセルをそのまま底面積の柱として扱う。柱は横方向へ広げず、固定高さへ押し出して高さ方向だけ減衰させる。試作段階では、サンプル間隔は 1km、加算前の強度は log 圧縮としてよい。
...
街灯系 glow の試作では、密度場は「レイ上のサンプル点ごとの夜間光ピクセル値の累積」として扱ってよい。補間や横方向の拡散は後回しにして、まずはざっくりした見た目の改善を優先する。必要な連続性は、描画時に 2°刻みの az サンプルから帯ポリゴンを組むことで確保してよい。
