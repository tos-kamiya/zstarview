...
- theme: Atlas 専用の羊皮紙テーマ。通常の `zstarview` からは選択できない。
- sky opacity: `0.0`。
- labels: 黒文字を基本にする。
...
- water surface: 有効。
- Earth guide: 有効。
- guidelines: 有効。水平線、方位ラベル、天の赤道、黄道、天頂・天底マーカーを濃い墨色またはセピア系の細線で描く。
- direction grid: guidelines 有効時は常時表示する。
- aircraft: 有効。
- satellites: 有効。
- cloud: 有効。ただし羊皮紙風背景で見える青灰色にする。
...
Atlas 向けには、通常のテーマとは分離した羊皮紙風テーマを first-class な内部スタイルとして持つ。
`object-white` は廃止し、通常の `zstarview` のテーマ選択肢にも Atlas の羊皮紙テーマを追加しない。

Atlas の羊皮紙テーマは次の性質を持つ。

- window background は淡い黄土色・ベージュ系の紙色とし、弱い質感や縁の暗化を専用背景描画で加えられる。
- sky disc は既定で無効。
- ラベルと HUD は黒または濃い茶色。
- ラベルアウトラインは原則不要。
- 補助レイヤーは濃いセピア、灰褐色、青灰色、水色を使い、線幅は細く調整する。
...
Atlas は濃い墨色またはセピア系の細い線を使い、黄道だけを点線、その他を実線とする。通常版の色付き・破線を使うガイドとは別の視認性設定にする。
ガイド線幅、グリッド線幅、マーカー線幅はスタイルから注入し、共有描画関数内の通常版向け固定値を直接変更しない。
...
Atlas の専用テーマでは `ThemeStyle.text` が黒または濃い茶色を返すため、レイヤー側が `label_rgb` を指定しないラベルは自然に暗色になる。
描画ルーチンへ Atlas 用の色や線幅を直書きしない。

レイヤースタイルの `outline_rgba` は文字用ではなく、線、点、十字マーカー、面、リボンなど図形そのものの縁取りまたは下敷き線を表す。
例えば、羊皮紙風背景の航空機軌跡では、必要な場合だけ濃い半透明の下敷き線を描き、その上に本体色の細線を描くことで視認性を補う。
...
4. 残った恒星のアウトラインを黒、濃グレー、または濃いセピアで描く。
5. 同じ恒星の本体を恒星色で描く。
...
2. Atlas 専用テーマ、暗色の細線、羊皮紙風背景を追加する。計画中。
