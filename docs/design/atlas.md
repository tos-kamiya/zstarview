...
- guidelines: 有効。水平線、天の赤道、黄道、天頂・天底マーカーを黒または濃いグレーのくっきりした細線で描き、方位ラベルと alt/az グリッドには通常版と同じ色相・彩度で明るさを抑えた緑系アクセント色を使う。
- direction grid: guidelines 有効時は常時表示する。
- Space キーの簡易表示では都市アウトラインを省略し、ラベルあり状態では共有の名前付き恒星ラベル描画を有効にする。
- aircraft: 有効。
- satellites: 有効。
...
Atlas は黒または濃いグレーの細い線を基本とし、方位ラベルと alt/az グリッドだけは通常版の緑系アクセント色を白背景向けに暗くした色で描く。黄道は点線、その他は実線とする。
...
明背景用の恒星描画では、通常の恒星に対しては現在の暗色の四角形下敷きと
恒星色の四角形本体を使う。`vmag <= 2.0` の明るい恒星については、四角形の
下敷きを使わず、ひし形のコントラスト層と明るい恒星用マーカーへ切り替える。
これにより、ひし形の周囲に四角い暗色の角が残ることを防ぐ。

描画順は次の通りとする。

1. 恒星本体より少し大きい暗色のひし形下敷きまたはアウトライン。
2. 恒星色の本体。
3. `bright_bodies` が `fill` の場合は塗りつぶしひし形、`outline` の場合は
   ひし形アウトラインの明るい恒星用マーカー。

この処理は Atlas の明背景用ラスターパスに限定し、通常の `zstarview` が使う
加算合成の星描画や、その通常描画のひし形処理を変更しない。明背景パスへ
`vmag`、明るい恒星の描画モード、ひし形のサイズ・線幅に関わる値を渡し、描画
キャッシュのキーにもこれらの視覚的入力を含める。特に `fill` と `outline` の
切り替えで古い画像を再利用しないことを保証する。
