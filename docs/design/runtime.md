...
- 同梱CAMS気候値は、アプリケーション資産としてバージョン管理された必須の静的データであり、通常実行時の外部通信キャッシュとは分けて扱う。参照キーは地点の緯度経度と暦月で、読み込み結果はプロセス内で再利用する。
- 同梱気候値の形式、必須フィールド、データ版、値域を起動時または初回参照時に検証する。検証に失敗した場合だけ、警告を記録したうえで空色計算へ清浄寄りの標準エアロゾルプロファイルを渡す。通常の地点選択でデータ取得や欠損時の通信フォールバックは行わない。
- 将来の任意のライブCAMS取得を実装する場合は、取得結果を同梱気候値とは別の地点キャッシュへ保存し、最大1時間の再取得抑制と24時間の stale fallback を適用する。通常版はこの取得経路を要求しない。
- CAMSプロファイルの状態は `bundled-climatology`、`cams-live`、`default` のいずれかとして記録する。認証情報、APIキー、個人設定の内容はログへ出力しない。

### 準静的レイヤーの通信失敗とフォールバック

- DEM は地点単位の単一ファイルではなく、要求された観測範囲を満たす DEM タイル集合をキャッシュ単位とする。
- DEM の各タイルは、正常に読める限り TTL 超過後も stale cache として利用できる。通信失敗時に必要なタイルが一部でも取得できない場合は、取得できたタイルだけで処理を継続できる設計としてよい。要求範囲に利用できるタイルが一つもない場合は地形地平線を生成しない。
- 水面は、観測地点を丸めた位置キー、要求半径、必要に応じた方位サンプル条件からなる scope cache を使う。通信失敗時はまず同一 scope のキャッシュを探し、なければ同一地点で要求範囲を包含する別半径のキャッシュを探す。
- DEM と水面の stale cache は、形式・必須フィールド・地理範囲が検証できる場合だけ利用する。JSON 破損、ハッシュ不一致、読込不能なタイルは invalid として利用しない。
- 水面の点列と境界線は、terrain horizon の DEM 計算完了後に生成する。水域ポリゴンだけを永続キャッシュし、生成した実行時の水面点には通常描画用の terrain occlusion alpha を付加する。これは地形プロファイルに依存する派生値なので永続化しない。ビューポート移動中の簡易描画ではこの値を無視する。
- stale cache を利用した結果は、通常の最新取得結果と区別できるよう、各レイヤーの状態へ `cache` ソースを記録する。キャッシュなしで取得に失敗した場合は `unavailable` として、そのレイヤーだけを非表示にする。
- UI の短い状態表示は `Terrain: cache`、`Terrain: unavailable`、`Water: cache`、`Water: unavailable` を使う。通信理由や例外の詳細、取得日時はログへ記録し、下部ステータス行へは出さない。
...
- 人工衛星の通常キャッシュ有効期間と通信失敗時の stale fallback 上限は、ともに24時間とする。失敗時は取得時刻から24時間以内の完全なキャッシュだけを `cache-stale` として返し、24時間を超えた場合は取得失敗として扱う。
