...
- 同梱CAMS気候値は、アプリケーション資産としてバージョン管理された静的データであり、通常実行時の外部通信キャッシュとは分けて扱う。参照キーは地点の緯度経度と暦月で、読み込み結果はプロセス内で再利用してよい。
- 同梱気候値の形式、必須フィールド、データ版、値域を起動時または初回参照時に検証する。検証に失敗した場合は空色計算へ清浄寄りの標準エアロゾルプロファイルを渡す。
