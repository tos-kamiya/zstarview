...
- 画面に固定の正方形2Dグリッド（Δu = Δv、45度回転）を置く。
  グリッド原点はビューポート左上。セル間隔は `target_stripes` で調整可能。
- 各セルの中心点で `CloudAltAzGrid.amount` をサンプルし、
  width スタイルと同じパーセンタイル正規化（非ゼロ値の12%ile/92%ile）を適用後、
  8段階の非線形量子化を行う。閾値は 1.4⁻ⁱ（i=7..1）。

  量子化テーブル（[0,1] を4等分）:

  | 雲量範囲    | レベル | 円直径（delta=30px時） |
  |-------------|--------|------------------------|
  | 0.000 – 0.095 | 0    | 描画なし |
...
- 円直径は `width_factor` でスケール可能。直径は `delta / 30 * wf / √2` で計算され、
  並行チェーン間の垂直距離（`delta / √2`）に対して隙間が確保される。
- レベル > 0 の各セルは、斜め右下（v 方向、grid index で `(i, j+1)`）の
  隣接セルと同じレベルであれば連結する。連結チェーンは `QPainter` +
  `RoundCap` ペンで太い線分（jellybean）として描画する。線幅 = 円直径。
- 孤立セル（連結先がない、またはレベルが異なる）は、セル中心に円を描画する。
  `drawPoint` + `RoundCap` ペン（幅 = 円直径）で実現。
