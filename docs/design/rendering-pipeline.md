...
- 描画段階では、`CloudAltAzGrid` から 3 つのストライプモードを生成できる。
  ...
  - `width`: 中心対称のストライプを描き、雲量を5段階に量子化して帯の見かけ幅を変える。
...
- 既定の雲描画スタイルは `width,50,0.85` とし、既定の雲不透明度は `0.10` とする。`COUNT` または `WIDTH` を `0` にすると雲描画を無効化する。
...
- `--cloud-stripe MODE[,COUNT[,WIDTH]]` の既定値は `width,50,0.85` とする。`COUNT` または `WIDTH` を `0` にすると雲描画を無効化する。
- 既定の雲不透明度は `0.10` とする。
