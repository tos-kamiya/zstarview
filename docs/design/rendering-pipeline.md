...
- 描画段階では、`CloudAltAzGrid` から 3 つのストライプモードを生成できる。
  - `jellybean`（既定）: 45° 傾いた画面固定の正方形グリッドを用い、各セルの雲量を 8 段階に量子化して円を描く。同じレベルが連続するセルは `RoundCap` ペンで太い線分（jellybean）として接続する。各形状の背後にほんの少し太く低アルファの輪郭を描き、雲の縁を薄く強調する。
...
- 既定の雲描画スタイルは `jellybean,24,1.0` とし、既定の雲不透明度は `0.05` とする。`COUNT` または `WIDTH` を `0` にすると雲描画を無効化する。
...
- `jellybean`（既定）
  ...
  - レベル > 0 の各セルは、斜め右下（v 方向、grid index で `(i, j+1)`）の隣接セルと同じレベルであれば連結する。連結チェーンは `QPainter` + `RoundCap` ペンで太い線分（jellybean）として描画する。線幅 = 円直径。
...
- `--cloud-stripe MODE[,COUNT[,WIDTH]]` の既定値は `jellybean,24,1.0` とする。`COUNT` または `WIDTH` を `0` にすると雲描画を無効化する。
- 既定の雲不透明度は `0.05` とする。
