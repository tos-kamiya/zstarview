...
- 既定の雲描画スタイルは `halftone,40,1.7` とし、既定の雲不透明度は `0.05` とする。`COUNT` または `WIDTH` を `0` にすると雲描画を無効化する。
...
- `--cloud-stripe MODE[,COUNT[,WIDTH]]` の既定値は `halftone,40,1.7` とする。`COUNT` または `WIDTH` を `0` にすると雲描画を無効化する。
- 既定の雲不透明度は `0.05` とする。
