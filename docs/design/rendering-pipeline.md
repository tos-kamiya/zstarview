...
- 描画段階では、`CloudAltAzGrid` から 3 つのストライプモードを生成できる。
  - `halftone`（既定）: 45° 傾いた画面固定の正方形グリッドを用い、各セルの雲量を 8 段階に量子化して円を描く。同じレベルが連続するセルは `RoundCap` ペンで太い線分（halftone）として接続する。各形状の背後にほんの少し太く低アルファの輪郭を描き、雲の縁を薄く強調する。
