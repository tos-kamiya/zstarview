...
- 恒星はsky snapshotで計算された離散的な位置を表示し、偶数秒tickで位置補間や恒星行列更新を行わない。sky workerの重いsnapshot計算と、惑星・航空機・人工衛星・瞬きの軽量更新は分離する。
...
- 偶数秒tickでは、準備済み惑星位置、航空機、人工衛星、瞬きを同じ対象時刻で更新した後、`request_client_update()`を1回だけ呼ぶ。
...
恒星位置は天体データ更新時の離散スナップショットとして扱い、表示tick間の
カメラ空間補間は行わない。恒星レイヤーは次の状態を別々に管理する。

- `dark_star_surface`
  - `vmag > 4.0` の暗い恒星を含む透明サーフェイス。
  - sky snapshotの生成時に更新し、表示tickでは画像を再生成しない。
- `bright_star_state`
  - `vmag <= 4.0` の恒星位置、サイズ、色、下敷き、ひし形描画に必要な入力。
  - 恒星サーフェイスはスナップショット時に生成してキャッシュし、表示時に
    viewportへ合成する。瞬きや簡易表示の恒星ラベルは、現在のsnapshotの
    画面座標へ直接描画する。
...
表示tickや表示位置を持つ軽量オーバーレイの再描画要求では、`request_client_update()` を使って Qt の repaint を予約する。
これは worker の sky snapshot 完了通知とは別の経路であり、Qt のイベントループによる要求の
集約を許容する。現在の `present-frame` cache を再利用する場合は、表示時刻や表示状態を表す既存のcache keyを使い、古いオーバーレイ画像を返さないようにする。
