...
- `star_interpolation_mesh`
  - 60秒区間の中間時刻から現在時刻まで（`-30`〜`+30`秒）の、約100px四方セルによる矩形メッシュ。`StarInterpolationMesh` が変換前後の頂点と行列数を一体で保持し、点座標の変換も提供する。
  - メッシュは最終 viewport の周囲にガード領域を加えた expanded 座標系で作る。暗い恒星サーフェイスも expanded viewport 全体を描画して保持し、クライアント座標の頂点を内部サーフェイス解像度へ縮小する。低解像度で全三角形を合成した後、その完成画像を元の viewport へ拡大し、viewport クリップでガード部分を隠す。
  - 恒星サーフェイスだけでなく、明るい恒星、asterism、twinkle、恒星ラベルなど同じ恒星移動に追随する全てのメッシュ利用者は、同じ expanded mesh と座標系を共有する。viewport 座標を受け取る点変換 API は、expanded 座標への移動と変形後の viewport 座標への復帰を内部で扱ってよい。
  - 偶数秒の表示tickで準備してキャッシュし、通常のpaint処理では頂点計算を繰り返さない。
  - メッシュは透明な空ディスク外も含む expanded 矩形全体を変形する。境界頂点を固定する処理は行わず、最終合成時だけ元の viewport へクリップする。ガード幅は cache key とメッシュ生成条件に含める。
  - メッシュの変更だけでは sky disc、terrain、cloud、urban などの重いキャッシュを無効化しない。

瞬きと簡易表示の恒星ラベルは、全画面サイズの一時サーフェイスを作らない。瞬きは対象点の画面座標、恒星ラベルはラベルアンカーをこのメッシュで変換し、変換後の位置へ直接描画する。これにより、少数の動的要素のために画面全体を再ラスタライズせず、ラベル文字も歪めない。
