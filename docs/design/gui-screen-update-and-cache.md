...
- 恒星補間では、60 秒ごとの sky snapshot 更新と、2 秒周期の恒星行列更新を分離する。行列更新は短命レイヤーの更新機会を利用するが、sky worker の重いスナップショット計算を再実行してはならない。
- 惑星、太陽、月は恒星補間とは別の2秒周期レイヤーとして、現在時刻の位置を専用workerで計算する。空色ディスク、雲の色、夜間光のキャッシュはこの更新で無効化しない。
