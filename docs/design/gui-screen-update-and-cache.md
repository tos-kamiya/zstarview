...
- 将来の恒星補間を導入する場合、60 秒ごとの sky snapshot 更新と、恒星行列の軽量な表示更新を分離する。行列更新は短命レイヤーの更新機会を利用してよいが、sky worker を再実行してはならない。
...
- `star_interpolation_transform`
  - スナップショット基準時刻から現在時刻までの 3x3 行列と、その算出に使った表示範囲・対応点条件。
