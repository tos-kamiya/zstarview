...
描画は、道路中心線に沿った約6000Kの白色ストロークと、中心線を約120m間隔でサンプリングした約3000Kの小さな光点の二層とする。ストロークは既定の道路光量の1/2程度で、日没後に有効化し、ローカル時刻に基づく深夜活動量係数で滑らかに減衰させる。光点は街路灯の抽象表現として夜間に残し、道路種別ごとの重要度に応じて光量、間隔、最大距離を調整してよい。いずれも実際の街路灯位置や交通量を意味するデータとして扱わない。
