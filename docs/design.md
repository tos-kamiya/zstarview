...
### Road proximity overlay

道路レイヤーは、都市アウトラインとは別の軽量な近傍補助レイヤーとして設計する。目的は道路地図を再現することではなく、観測地点のすぐ近くにある幅の確認可能な道路形状を参考情報として表示することである。

- ソースはOverture Transportationの`subtype=road` segmentを基本とする。
- 検索・取得の論理的な描画半径は観測地点から地表距離50mとする。segmentの端部を取りこぼさないため、取得bboxには小さな余白を設け、取得後に地表距離50mの円でクリップする。
- `width_rules`を持つsegmentだけを採用する。`class`、`subclass`、道路種別、平均的な道路幅からの補完値は使わない。
- `width_rules`の`between`がある場合は、segmentを線形参照区間で分割して幅を割り当てる。区間ごとの幅を中心線の左右へ展開し、地表上の帯状形状として保持してから既存の観測地点基準座標へ変換する。
- 幅のない区間、解釈不能な幅ルール、50m円と交差しない区間は派生キャッシュへ保存しない。
- 描画では帯状形状を直接細密に保持せず、投影後の視認性に必要な頂点だけへ簡略化してよい。これにより近距離でも道路が画面を塞ぎにくくなる。
- 採用データが0件の場合は正常な空レイヤーとし、中心線だけのフォールバックや推定幅による表示は行わない。
- fast-modeまたは簡易表示中は、道路データの取得・幅展開・投影・描画を省略する。通常表示へ戻ったときに必要なら通常のキャッシュ取得・描画経路を再開する。
- Overtureの道路segmentは中心線、`level_rules`、幅、道路種別を別属性として持つため、高架・トンネルなどの上下関係は幅とは分離して扱う。初期実装では`level_rules`を保持し、地形・天球投影上の厳密な遮蔽処理は後続課題とする。
- キャッシュキーには観測地点、50m半径、Overture release識別子、道路幅処理方式のバージョンを含める。道路幅の解釈を変更した場合は既存キャッシュを再利用しない。
