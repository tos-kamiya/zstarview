...
B13/B16 によるシェル配分の較正前段階では、通常経路の B13/C13 入力と
描画結果を変更しない。診断用の同伴バンド取得は、GOES では C13 と同じ
...
`dev-samples/diagnose_b13_b16.py` に隔離する。
...
strength = min(calibrated_strength, 0.25)
final_weights = mix(legacy_weights, candidate_weights, strength)
```

さらに、各セルでシェル間を移動できる雲量は `amount13 * 0.20` 以下とする。
ここで移動量は、補正前後のシェル量の差の絶対値合計を2で割った値と定義する。
補正の前後でシェル量の総和を保存し、各量を非負とする。主な移動は5 km
シェルと隣接する3 km / 7 kmシェルの間で行い、3 kmから7 kmへの直接移動は
行わない。`0.25` の混合上限と `0.20` の移動上限は両方を適用し、厳しい方を
最終結果に採用する。具体的なスコア曲線と候補重みは診断データの追加検証後に
決める。
...
通常GUIの提示経路では、シェル合成時に透明画素を演算対象から除外する。また、表示中心、FOV、内部描画サイズ、雲グリッド、欠損マスク、描画方式、色、不透明度が一致する間は合成済みの雲ラスターを再利用する。HUDだけが変化した再提示で、変化していない3シェルを再投影・再合成しないためである。雲データ更新、表示操作開始、または描画設定変更に伴う compositor の無効化では、このラスターキャッシュも破棄する。
