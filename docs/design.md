...
- 地形地平線プロファイル
  ...
  - 地点依存、時刻非依存のデータとして扱う
- 補助稜線プロファイル
  - 方位ごとの局所極大値をヒステリシス付きで抽出し、短い polyline 群として保持してよい
  - 主地平線のような 1 点/方位 の包絡線ではなく、山頂や稜線の重なりを薄く見せる補助表現として扱う
  - DEM ノイズを拾いすぎないよう、最小仰角差・最小下降量・最小距離間隔などの閾値を持ってよい
  - 同一方位内では、より近い副稜線がより高く見えている場合、より遠くて低い副稜線は表示しない
  - 逆に、遠方の副稜線が前景より高く見える場合は、その遠方ピークを残してよい
- 描画方針
  - 主地平線は現行の単一シルエット線を維持する
  - 補助稜線は別レイヤーとして低 opacity・細線で描画してよい
