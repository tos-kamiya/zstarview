...
PLATEAU建物形状の詳細度は、まずLOD0/LOD1を対象に実験し、効果と表示密度を確認した後にLOD2へ拡張する段階方式とする。初期実装では建物ごとに利用可能なLODのうち最大1を選択し、LOD1がなければLOD0へフォールバックする。LOD2は次段階で、LOD1の結果が有効であることを確認した場合だけ追加する。LOD3とLOD4は、空を背景とした建物シルエット表示に対してデータ量と処理量が過大になるため対象外とする。PLATEAUのCityGML APIが提供するLOD別地物数を、対象データの詳細度確認に利用してよい（[CityGML API](https://docs.plateauview.mlit.go.jp/api/rest/operations/datacatalogcitygmlconditions/)）。

派生タイルの建物レコードは、従来の外周リング・高さ・範囲を後方互換の基本情報として保持する。初期のLOD1実験では、壁面を多数の線として直接描画せず、単純立体の外側を観測地点から見たシルエットへ変換し、既存の屋根ライン表示と同じ描画経路へ渡す。これにより、表示の混雑を抑えながら建物部分ごとの高さや外形の効果を検証する。LOD2へ進む場合は、屋根面・壁面などの詳細サーフェスと頂点標高を追加し、同じシルエット計算へ入力する。遠距離または低層建物では、距離に応じた頂点・面の間引きとLOD1/LOD0への縮退を許可する。
...
形状情報を追加する派生タイルでは、`DERIVED_TILE_SCHEMA_VERSION`を2へ更新し、`geometry_lod`、`geometry_mode`、LOD別建物数をメタデータまたはタイルレコードへ保存する。LOD2へ進む場合は、さらに`surfaces`、`max_geometry_lod`、詳細サーフェス数を保存する。スキーマ1のキャッシュは新しい形状方式に対応済みとはみなさず、準備CLIの再実行時に再生成する。キャッシュ更新判定では、既存の整備年度・登録年度・仕様・ファイル数・ファイルサイズの照合を引き続き適用する。

形状方式の実験対象は、東京タワーを含む港区の自治体コード`13103`とする。まず`13103`のCityGMLカタログでLODの提供状況を確認し、LOD1が存在する建物について派生タイルを生成する。東京タワー周辺では、Overture Mapsの従来表示、PLATEAUのLOD0、PLATEAUのLOD1シルエットを比較し、輪郭の再現性、画面の混雑、変換時間、キャッシュ容量、起動後の描画時間を評価する。LOD1で効果が不足する場合に限り、同じ実験対象へLOD2を追加する。
