...
取得したwayは観測地点中心のローカルXYへ変換し、3kmの内側円と10kmの外側円による環状範囲へクリップする。wayの線分が距離円を横切る場合は交点を補間し、表示対象部分だけを残す。10kmの要求範囲は水域境界線の表示範囲と揃え、3km未満を除外することで、観測地点近傍の道路光が画面を過度に占有することを防ぐ。
