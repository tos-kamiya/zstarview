...
描画は、道路中心線に沿った約6000Kの白色ストロークと、中心線を約120m間隔でサンプリングした約3000Kの小さな光点の二層とする。ストロークは既定の道路光量の1/2程度で、既存の`night lights`と同じ太陽高度フェードを適用したうえで、ローカル時刻に基づく深夜活動量係数で滑らかに減衰させる。光点は夜間判定中のみ、既定の道路光量の約4割で一定に描画し、深夜の時刻活動係数は適用しない。いずれも実際の街路灯位置や交通量を意味するデータとして扱わない。
