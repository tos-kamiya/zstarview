...
- 補助稜線プロファイル
  ...
  - 距離帯の境界は `2 / 4 / 8 / 16 / 32 / 64 / 128 km` を初期値として扱ってよい
  - 主地平線のような 1 点/方位 の包絡線ではなく、近景・中景・遠景の層を分けて見せる補助表現として扱う
  - 補助稜線は遠景ほど少し細くしてよい
  - 遠景のぼけは、薄く太い下地線を先に描いてから本線を重ねることで近似してよい
- 描画方針
  - fast-mode では主地平線の単一シルエット線を維持し、通常描画では補助稜線の帯線のみを出してよい
  - 補助稜線は別レイヤーとして描画し、同色でよい
  - 帯線の色は主地平線と同じ色を使い、遠景ほど本線を細くしてよい
  - 遠い帯ほど下地線の太さを少し強めて、ぼやけた遠景に見せてよい
  - 帯線の alpha も最前面帯を fast-mode に近づけ、遠景ほど弱めてよい
...
- 縦幅のしきい値は、画面上で見分けがつかない程度の薄さを基準にし、距離や FOV に応じて固定ピクセル閾値ではなく投影後の高さで判定してよい。
...
- 地形地平線の距離帯は `2 / 4 / 8 / 16 / 32 / 64 / 128 km` を初期値として扱い、最遠帯までの計算は `128 km` を基準に行ってよい。
