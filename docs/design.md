...
ダウンロードは専用CLI `zstarview-download-coastline` だけが行う。通常のGUI起動時に自動ダウンロードしてはならない。CLIは `--lon-min` と `--lon-max` で指定された経度範囲に重なるx列を選択し、グリッド境界まで範囲を拡張して必要なアセットを取得する。全世界を取得する場合は `--all` を使う。既定のデータ版はGitHub Releaseタグ `coastline-data-20260725` であり、`--cache-dir` でキャッシュの基底ディレクトリを変更できる。将来の25m水面マスクも同じCLIのオプションで取得できるようにし、海岸線と水面マスクのダウンロード入口を分散させない。
...
125m、250m、500mの水面マスクはアプリに同梱する既存経路として維持する。海岸付近の表示位置を改善するための25m水面マスクは、アプリ本体には同梱せず、GitHub Releaseから利用者が明示的に取得する任意データとする。未取得、取得失敗、破損時には25m経路を無効にし、既存の125m、250m、500mの解像度へフォールバックする。
...
CACHE_PATH/water/
  osm-water-polygons/20260725/schema-1/resolution-25m/
    manifest.json
    READY
    water-grid-x00-20260725.zip
    water-grid-x01-20260725.zip
```

配布アセットは既存の32×16全球グリッドのx列単位とする。各ZIPには該当x列の全y行について、`tile_yNN_xNN.tif`、水面が全くないタイルの`.0`、全面水面のタイルの`.1`を格納する。配布単位とタイル境界は独立しており、経度範囲から必要なx列を選択できるよう、manifestに列番号、範囲、サイズ、SHA-256、データ日付、CRS、解像度を記録する。

25m ZIPは展開済みTIFFのディレクトリを通常キャッシュとして作らない。アプリは必要なx列のZIPを開き、必要なTIFFエントリだけをメモリ上で読み込んで水面判定を行う。全タイルを画素配列へ一括展開せず、GDALのウィンドウ読み込みまたは同等の部分読み込みを使い、常時メモリ使用量を必要なタイル数に限定する。これにより、約137MBの配布ZIPを保持しつつ、展開後の約1GB規模のディスク占有を避ける。

25m水面マスクのダウンロードは `zstarview-download-coastline --water-25m` で有効化し、`--lon-min` と `--lon-max` に重なるx列だけを取得する。`--all --water-25m` は全x列を対象とする。ダウンロード中は既存の海岸線アセットと同じ進捗表示、サイズ検証、SHA-256検証、一時ファイル、`READY` マーカーの手順を使う。ZIPは展開せず、検証済みの完成ファイルをバージョン付きキャッシュへ配置する。

25m経路は海岸付近の水面ドットの位置改善を目的とし、川・湖の境界ベクタデータや内陸水域のOverpass取得経路を置き換えない。25mラスタとベクタ海岸線の位置が一致しない場合もあるため、25mを導入しただけで海岸線の幾何精度が保証されるとは扱わない。
