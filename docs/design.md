...
- INPROGRESS: 地平線方位マーカーを「縦線」から「円盤」に変更
  - 背景:
    - 画面相対の縦線マーカーは、天頂付近や低仰角時に不自然な見え方になりやすい。
  - 仕様案:
    - 方位マーカーは線分ではなく、地平線上の方位点（N/NE/E/SE/S/SW/W/NW）に小さな円盤を配置する。
    - 円盤半径は画面半径 `R` に対して `r = clamp(R * 0.012, 2px, 6px)` とする。
    - 色は地平線ラインと同色、塗りつぶしを基本とし、必要なら1pxの薄い縁取りを許容する。
    - 方位ラベルとの干渉時は、ラベル側を外周方向へ `+4px` オフセットする。
  - 受け入れ観点:
    - 天頂表示（例: `A=90`）で「縦線」に見えず、方位位置が点として認識できる。
    - 低仰角表示（例: `A=5`）でも不自然な傾き表現が発生しない。
