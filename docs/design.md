...
Overpassへの要求は道路種別ごとに分けず、観測地点から10kmの範囲を正規表現フィルタで一括取得する。応答のwayとノード列は、取得後に`highway`値ごとのグループへ分ける。要求半径、対象タグ集合、データスキーマのバージョン、観測地点を正規化したキャッシュキーを使い、同じ範囲の再表示で外部APIへ再要求しない。失敗時は有効な既存キャッシュを優先し、キャッシュもない場合はレイヤーを省略する。
