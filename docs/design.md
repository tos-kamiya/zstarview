...
海岸線の近距離境界を高密度に描画するため、OSM Water Polygons の海岸ポリゴンから生成した静的ベクタタイルを、既存の `water_overlay` キャッシュとは分離して保持する。これは現時点では実装前のキャッシュ設計であり、現在のアプリはこのキャッシュを参照しない。

既存の32×16全球グリッドをトップレベルのアドレスとして使い、通常タイルと、容量が大きいタイルだけを4×4に分割した子タイルを混在させる。親タイルを子タイルへ分割した場合、親の重いデータファイルは保存せず、manifestだけで子タイルの存在を示す。
...
`manifest.json`には、データソースと更新日、座標系、32×16グリッド、子タイル分割係数、各トップレベルタイルの `single`/`4x4` 配置、ファイルサイズ、SHA-256を保存する。子タイルの境界で線が途切れないよう、生成時に境界上の線分を隣接タイルへ少し重複させるか、クリップ端点を連結可能な形で保存する。

アプリはmanifestを先に検証し、観測地点のトップレベルタイルが通常配置なら単一ファイルを、分割配置なら10kmの検索範囲に重なる子タイルだけを読む。部分取得は一時ファイルへ行い、ハッシュとGeoJSONメタデータの検証後に確定キャッシュへatomic renameする。未取得・不完全・ハッシュ不一致のファイルを有効な海岸線キャッシュとして扱わない。

海岸線ベクタタイルは海岸境界専用であり、川・湖・池などの内陸水域は既存のOverpass経路で扱う。GeoJSON試作では1タイルが未圧縮252MB、4×4分割後も最大約73MBとなったため、座標量子化・形状簡略化・gzipまたはバイナリベクタ形式は、実装時に別途評価する。
