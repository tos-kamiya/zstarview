...
- 夜間の人工照明には、太陽高度による薄明フェードとは別に、観測地点のローカル市民時に基づく滑らかな深夜活動量係数を適用してよい。初期プロファイルは19〜21時を`1.00`、22時を`0.85`、0時を`0.65`、2時を`0.50`、4時を`0.45`、5時頃を`0.50`〜`0.60`の目安とする。
- 深夜活動量係数は night light のbase layerと屋根ポリゴンの面塗りへ適用する。edge/ridge glowは自然の大気光としてこの時計時刻係数から除外する。
- AKARI IR bandsは深夜活動量の低下に対応して、色変換後の最終opacityを既定設定で`0.05`〜`0.10`の範囲へ滑らかに変化させる。`akari_ir_bands_opacity`は深夜2〜4時頃の最大実効opacityとして扱い、通常時はその約`1/2`まで下げる。これは静的なAKARIデータへの視覚的補正であり、データの時間変化ではない。
- 夜間のsky discには、同じ深夜活動量カーブを使ってアンビエントRGB `[1, 2, 5]` を `[2, 4, 10]` まで滑らかに持ち上げる。これはAKARIや人工照明とは独立した、完全な黒を避けるための空色成分である。
...
- 建物アウトラインの閉じた屋根ポリゴンは、輪郭線とは別の面塗り成分として扱う。面塗りの最終強度へnight lightと同じ深夜活動量係数を単純に乗算し、最低アルファの例外は設けないため、深夜には面塗りが消えてよい。輪郭線は地物可読性のため、面塗りとは独立した強度で残す。
