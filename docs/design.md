...
各表示点では濃い青の実線を下地に薄い青の細い点線を重ねた平行斜線を、画面空間で右上がりに並べる。
線端は角型とする。降水強度は線の本数、
距離は線の高さと不透明度で表す。最終 alpha はレイヤー全体の opacity に
`opacity_factor` を乗算する。表示角は物理的な降水高度ではない。線幅と間隔には viewer
の line-width scale を乗算する。濃い青の基準線幅は `2.2 px`、薄い青の点線はその
`0.4` 倍で、ダッシュパターンは `[0.4, 5.6]` とする。両色とも距離補正後の alpha の
`0.7` 倍を使い、範囲外の距離は両端へ clamp する。
