...
- 地形地平線ポリライン統合
  - 目的:
    - `dev-samples/horizon_polyline_demo.py` の DEM レイスキャンを本体へ統合し、既存の幾何学的地平線に加えて、観測地点依存の地形地平線を地面色ベースのポリラインで重畳表示する。
  - 確定スコープ:
    - 既存の `alt=0` 地平線と方位ラベルは維持する。
    - 地形地平線は追加線として描画し、地面/空の塗り分け境界にも利用する。
    - 地形地平線の有効/無効は opacity で制御し、`opacity == 0` のときは DEM 取得・計算・描画を一切行わない。
  - CLI / GUI:
    - CLI には地形地平線 opacity を与える引数を追加する。
    - デフォルト値は非ゼロとし、デフォルトで有効とする。
    - GUI には `Terrain Horizon` トグルを追加し、内部的には `0` と既定非ゼロ opacity を切り替える。
    - CLI 起動時に opacity `0` が指定された場合、その起動中は GUI から再有効化できないようにする。
  - データ取得/キャッシュ:
    - DEM は Copernicus DEM を利用し、既存依存（`boto3`, `rasterio`, `pyproj`）で取得/読込する。
    - 永続化するのは DEM データのみで、地形地平線ポリライン自体は保存しない。
    - DEM がローカルキャッシュ済みなら、アプリ起動時または有効化時にポリラインを再計算する。
  - 実行モデル:
    - UI スレッドはブロックしない。
    - 地形地平線は雲と同様のタイミングでバックグラウンド更新を開始する。
    - 地形地平線は地点依存・時刻非依存なので、計算結果は `(alt, az)` プロファイルとして保持し、視線変更時は再取得せず再投影のみを行う。
  - 失敗時挙動:
    - DEM 取得失敗時は bottom status line に terrain 用の失敗状態を表示する。
    - 失敗状態は当該セッション中は維持し、自動再試行しない。
    - 次回起動時にのみ再取得を試みる。
  - モジュール分割:
    - `src/zstarview/terrain/dem.py`
      - Copernicus DEM ダウンロード
      - GeoTIFF 読み込み
      - モザイク生成とサンプリング
    ...
    - `src/zstarview/ui/terrain_state.py`
      - 直近の terrain horizon 点列と banner/status 状態
  - 実装メモ:
    - DEM 見かけ高度計算には地球曲率と大気屈折を含める。
    - 地形地平線ポリラインは既存の天球投影へそのまま載せ、terrain だけの縦方向強調は行わない。
    - 0度未満の地形見かけ高度も許容し、幾何学的地平線より下に描画されうる。
  - 受け入れ観点:
    - デフォルト起動時に terrain horizon が非同期に取得/計算され、取得成功後は既存地平線に加えて terrain の折れ線が表示される。
    - opacity `0` 指定起動時は DEM 取得も描画も走らず、GUI から再有効化できない。
    - DEM 取得失敗時は status line に継続表示され、同一セッションで自動再試行しない。
