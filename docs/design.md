...
描画は、道路中心線に沿った約6000Kの白色ストロークと、中心線を約120m間隔でサンプリングした約3000Kの小さな光点の二層とする。ストロークは`road_light_opacity`を基準光量として、既存の`night lights`と同じ太陽高度フェードを適用したうえで、ローカル時刻に基づく深夜活動量係数で滑らかに減衰させる。光点は夜間判定中のみ、ストローク基準光量の約0.8倍で一定に描画し、深夜の時刻活動係数は適用しない。これにより、従来のストローク0.5・光点0.4の相対比を維持したまま、基準opacityを有効に使う。いずれも実際の街路灯位置や交通量を意味するデータとして扱わない。
