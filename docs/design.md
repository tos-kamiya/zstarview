...
街灯の最初の帯は例外的に、観測地点直近で消えやすいのを避けるため、下端基準を 250m 側の副稜線に寄せる。これにより、高い観測点でも最初の帯が 250m から 500m の間で描かれやすくなる。
...
将来的には、街灯の帯と ridge glow をいったんフル解像度のポリゴンから切り離し、`1/4` 程度のオフスクリーン `GlowMask` へ rasterize してから、ぼかしと色付けを経て `RGBA` に戻して合成する。`GlowMask` は `float32` の alpha だけを持つ。これにより、帯ごとの積み上げ表現から連続的な発光場へ移行でき、CLI の明るさ係数も float のまま最後まで保持しやすくなる。グロー層は scene/state には含めず、`SkyCompositorCache` の合成用中間画像として扱う。
この方式では、幾何の輪郭を作るシャープ層と、にじみを作るグロー層を分ける。前者は現在の稜線や帯ポリゴンを担い、後者は夜間光と ridge の発光をまとめて保持する。最終的な見た目調整は、低解像度の alpha マスクに対する平滑化と、固定色での tint で行う。
