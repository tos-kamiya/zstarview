...
将来的には、街灯の帯表現をフル解像度のポリゴンから切り離し、`GlowMask` へ rasterize してから合成する。`GlowMask` は `float32` の alpha だけを持つ。街灯側は、`az` ごとにレイ上の等間隔サンプルを置き、その地点が属する夜間光タイルのピクセル値をそのまま柱の強度として足し込む。柱は横方向に広げず、タイルピクセルと同じ底面積のまま固定高さへ押し出し、高さ方向だけ減衰させる。試作段階では距離サンプル間隔を 1 km とし、加算前に log 圧縮をかける。最終的な可視化は、各レイで可視な部分だけを積算した密度場として扱い、手前の稜線や地平線より下は切り落とす。
