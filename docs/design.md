...
- 補助稜線プロファイル
  ...
  - 距離帯の境界は `3.75 / 7.5 / 15 / 30 / 60 / 120 km` を初期値として扱ってよい
  - 主地平線のような 1 点/方位 の包絡線ではなく、近景・中景・遠景の層を分けて見せる補助表現として扱う
  - 補助稜線はまず固定幅で試し、見た目の整理より層構造の把握を優先してよい
- 描画方針
  - fast-mode では主地平線の単一シルエット線を維持し、通常描画では補助稜線の帯線のみを出してよい
  - 補助稜線は別レイヤーとして描画し、初期の試し表示では同色・帯ごとの段階的な太さでよい
  - 帯線の色は主地平線と同じ色を使い、最前面帯は fast-mode と同じ太さを起点にし、遠景の帯ほど細くしてよい
