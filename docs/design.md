...
25m ZIPは展開済みTIFFのディレクトリを通常キャッシュとして作らない。アプリはZIP内の必要なTIFFエントリだけをメモリ上で読み込んで水面判定を行う。全タイルを画素配列へ一括展開せず、GDALのウィンドウ読み込みまたは同等の部分読み込みを使い、常時メモリ使用量を必要なタイル数に限定する。これにより、約137MBの配布ZIPを保持しつつ、展開後の約1GB規模のディスク占有を避ける。
