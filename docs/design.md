...
アプリはmanifestを先に検証し、観測地点のトップレベルタイルが通常配置なら単一ファイルを、分割配置なら6kmの検索範囲に重なる子タイルだけを読む。部分取得は一時ファイルへ行い、ハッシュとGeoJSONメタデータの検証後に確定キャッシュへatomic renameする。未取得・不完全・ハッシュ不一致のファイルを有効な海岸線キャッシュとして扱わない。
