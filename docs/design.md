...
PLATEAU派生キャッシュの完了判定はディレクトリの存在だけに依存しない。`tile_index.json`、対象範囲、データソース、自治体コード、データ年度、変換完了状態を持つメタデータを検証し、観測地点を含むキャッシュだけを選択する。
...
選択順は、観測地点に対応する有効なPLATEAU派生キャッシュ、通常のOverture派生キャッシュ、最後にOvertureのオンデマンド取得である。
