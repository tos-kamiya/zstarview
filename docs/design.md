...
- 地形地平線ポリライン統合
  - 目的:
    - `dev-samples/horizon_polyline_demo.py` の DEM レイスキャンを本体へ統合し、既存の幾何学的地平線に加えて、観測地点依存の地形地平線を緑色ポリラインで重畳表示する。
  - 確定スコープ:
    - 既存の `alt=0` 地平線、方位ラベル、地面/空の塗り分けは変更しない。
    - 地形地平線は追加線としてのみ描画する。
  ...
  - 実装メモ:
    ...
    - 緑ポリラインは既存の天球投影へそのまま載せ、terrain だけの縦方向強調は行わない。
    - 0度未満の地形見かけ高度も許容し、幾何学的地平線より下に描画されうる。
  - 受け入れ観点:
    - デフォルト起動時に terrain horizon が非同期に取得/計算され、取得成功後は既存地平線に加えて緑の折れ線が表示される。
