...
将来的には、街灯の帯表現をフル解像度のポリゴンから切り離し、`GlowMask` へ rasterize してから合成する。`GlowMask` は `float32` の alpha だけを持つ。街灯側は、レイ上に等間隔のサンプル点を置き、その地点が属する夜間光タイルのピクセル値をそのまま柱の強度として足し込む。柱は横方向に広げず、タイルピクセルと同じ底面積のまま固定高さへ押し出し、高さ方向だけ減衰させる。試作段階ではサンプル間隔を 1km とし、加算前に log 圧縮をかける。これにより、板状の帯ではなく、地表から立ち上がる発光柱として見せやすくなる。見た目の連続性が必要なときは、柱を線で埋めるのではなく、2°刻みの az サンプルから下端と上端を結ぶ長い帯ポリゴンとして描画する。
