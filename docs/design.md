...
- 昼間は全シェルを白基準で描画する。
- 太陽高度が薄明の範囲へ入った場合、太陽方向とシェル高度から暖色係数を計算する。
- 高層シェルは夕焼け光を受けた雲として橙〜赤の寄与を許容する。
- 低層シェルは、暖色を弱めた暗い灰赤色を基本とし、雲全体を一様に赤くしない。
- 夜間は暖色係数をゼロへ戻し、既存の太陽高度ベースの青灰色 tint を使う。
- 空色ディスクのピクセル色をそのまま雲色へ転用しない。空色の夕焼け色と雲の照明色を分離して、夕焼け域の過剰な赤化を防ぐ。
