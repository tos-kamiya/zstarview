...
- AKARI IR bandsは深夜活動量の低下に対応して、色変換後の最終opacityを既定設定で`0.10`〜`0.15`の範囲へ滑らかに変化させる。`akari_ir_bands_opacity`は深夜2〜4時頃の最大実効opacityとして扱い、通常時はその約`2/3`へ下げる。これは静的なAKARIデータへの視覚的補正であり、データの時間変化ではない。
