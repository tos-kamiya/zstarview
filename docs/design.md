...
ダウンロードは専用CLIだけが行う。通常のGUI起動時に自動ダウンロードしてはならない。CLIは `--lon-min` と `--lon-max` で指定された経度範囲に重なるx列を選択し、グリッド境界まで範囲を拡張して必要なアセットを取得する。全世界を取得する場合は `--all` を使う。

ダウンロードしたアセットは一時ファイルへ保存し、サイズとSHA-256を検証する。展開時は絶対パス、`..`、シンボリックリンクなどの危険なエントリを拒否する。全ファイルの展開と検証が完了した後にだけ `READY` を作成し、既存キャッシュを壊さないよう、一時キャッシュからatomic renameまたは世代切り替えを行う。未取得・不完全・ハッシュ不一致のファイルを有効な海岸線キャッシュとして扱わない。
