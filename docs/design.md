...
- [INPROGRESS] 惑星表示のスタイル変更（記号 -> クロスゲージ）
  - 目的:
    - 惑星記号（emoji）表示を廃止し、恒星ラベルとの視認競合を減らす。
    - 太陽・月と同系統の記号（クロスゲージ）で視覚言語を統一する。
  - 仕様案:
    - 惑星（Mercury〜Pluto）は画面上でクロスゲージ表示に統一する。
    - マウスカーソルを惑星近傍へ移動したときのみ、惑星名をツールチップ相当で表示する。
    - 通常時は名称テキストを常時描画しない（画面の混雑を抑制）。
  - 実装メモ:
    - `render/draw.py` の `draw_solar_system_bodies()` で、惑星分岐の `draw_outlined_text(... body.symbol ...)` をクロスゲージ描画へ置換する。
    - 既存のハイライト判定（`find_highlighted_object`）を流用し、`PlanetBody` の場合は `name` を優先表示する。
    - 太陽は既存どおりクロス、月は既存どおり位相ディスク+クロスを維持する。
  - 段階導入:
    - PR1: 惑星の常時記号描画を廃止し、クロスゲージへ置換
    - PR2: 惑星ホバー時の名称表示を追加（閾値と表示位置の調整を含む）
