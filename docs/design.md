...
共通基底は現行版 `zstarview/1.32.3` で、各クライアントは短い suffix を足して区別する。
将来の版更新時は、`zstarview/<current-version>` の基底部分だけを差し替えればよい。

- `build_user_agent("water-overlay")` -> `zstarview/1.32.3 (+water-overlay)`
- `build_user_agent("nominatim")` -> `zstarview/1.32.3 (+nominatim)`
- `build_user_agent("night-lights")` -> `zstarview/1.32.3 (+night-lights)`
- `build_user_agent("overture-release")` -> `zstarview/1.32.3 (+overture-release)`
- `build_user_agent("geosatellite")` -> `zstarview/1.32.3 (+geosatellite)`
- `build_user_agent("tropical-cyclone")` -> `zstarview/1.32.3 (+tropical-cyclone)`
- `build_user_agent("ip-api")` -> `zstarview/1.32.3 (+ip-api)`
- `build_user_agent("opensky")` -> `zstarview/1.32.3 (+opensky)`
- `build_user_agent("satellites-celestrak")` -> `zstarview/1.32.3 (+satellites-celestrak)`
- `build_user_agent("satellites-horizons")` -> `zstarview/1.32.3 (+satellites-horizons)`
- `build_user_agent("satellites-wheretheiss")` -> `zstarview/1.32.3 (+satellites-wheretheiss)`
- `build_user_agent("copernicus-dem")` -> `zstarview/1.32.3 (+copernicus-dem)`
- `build_user_agent("s3")` -> `zstarview/1.32.3 (+s3)`
- `build_user_agent("skyfield-loader")` -> `zstarview/1.32.3 (+skyfield-loader)`
