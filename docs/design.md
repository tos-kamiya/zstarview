...
- 高密度恒星表示（`vmag <= 10`）の操作応答改善（確定仕様）
  - インタラクションモード:
    ...
    - インタラクション中の星描画は `draw_vmag_limit` により軽量化する。
  - キー操作時の更新フロー:
    - まず既存キャッシュデータで即時再描画して操作遅延を抑える。
    - その直後に `vmag <= 6` の星データ再生成をバックグラウンドへ要求する。
    - ワーカーが実行中の場合は要求を保留し、完了直後に最新視点で追従更新を1回実行する。
  - LOD6専用データ:
    - 起動時に `star_catalog_lod6`（`vmag <= 6`）を前計算して保持する。
    - LOD6更新ではこの専用データを直接使い、`-V10` など高上限時の毎回フィルタコストを避ける。
  - 通常品質への復帰:
    - インタラクション終了時に通常上限（ユーザー指定 `vmag_limit`）で再計算を要求し、完全データへ復帰する。
  - クラウド更新との整合（描画割り込み抑止）:
