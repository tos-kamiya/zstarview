...
- 最終的なAKARI opacityは連続的に変化させ、既定設定では通常時を `0.10`、深夜2〜4時頃を `0.15` とする。
- `--akari-ir-bands-opacity` で指定する値は、深夜2〜4時頃の最大実効opacityとなる。通常時はその値の約 `2/3` まで滑らかに下がる。
