...
- 屋根ポリゴンの面塗りは人工照明の一部として扱い、基準アルファを1.2倍したうえで、night light と同じ太陽高度係数を最終的な塗り強度へ乗算する。太陽高度 `-4` 度で消灯し、`-9` 度で最大となるよう、その間を滑らかに補間する。太陽が最低高度を通過した後の上昇側では、最低高度から `-9` 度へ向かう間に活動係数を100%から45%まで滑らかに減衰させる。
- 太陽高度係数の適用により、屋根ポリゴンの面塗りは完全に消えてよい。面塗りを維持するための最低アルファ値を設けない。
