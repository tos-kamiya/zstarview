...
#### 6.4.1 恒星の時間更新と滑らかな表示（将来方針）

- 恒星、惑星、空色ディスクの天文計算は、既定では 60 秒間隔のスナップショット更新を維持する。
- 60 秒のスナップショット間では、恒星の再計算を毎回行わず、前回スナップショットからの経過時間に応じた軽量な表示補間を行ってよい。
...
- この補間表示は、恒星の表示位置を滑らかにするためのものであり、60 秒スナップショットの天文計算精度や惑星・空色ディスクの更新間隔を変更するものではない。
- この節の補間表示は将来方針であり、現行実装が 60 秒ごとのベースフレーム切り替えを行うことを妨げない。
