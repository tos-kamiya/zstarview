...
- エアロゾルの散乱量は、CAMS（Copernicus Atmosphere Monitoring Service）から抽出した、全球約80 km（ADSの0.75度格子）・月別AOD550の静的データを利用できる。アプリには空色計算に必要な項目だけを抽出した軽量な派生データを同梱し、CAMSの元データを同梱しない。
- 同梱データは過去の解析値・観測値を平均した気候値であり、現在の天気や将来の予測値を表さない。観測地点の緯度経度と暦月から対応セルを選び、空色計算へ適用する。
- 通常のアプリ実行では、CAMS APIへの接続、アカウント作成、API認証情報の設定を要求しない。
- 同梱データが読み込めない場合、または対象地点を含まない場合は、やや清浄寄りの標準固定大気プロファイルへフォールバックする。
- フォールバック用の標準プロファイルは、過度に霞んだ状態ではなく、やや清浄寄りの大気を基準とする。
