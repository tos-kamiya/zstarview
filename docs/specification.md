...
- 道路は建物・水面・地形を補助する参考情報として表示してよい。
- 道路の対象範囲は観測地点から地表距離50m以内とする。
- 取得時は道路segmentの端部が対象範囲にかかる場合を扱えるよう、50mより広い範囲を取得してよい。その後、描画対象を50m以内へクリップする。
- 道路データはOverture Transportationのroad segmentを利用してよい。
- `width_rules`が存在する道路区間だけを描画対象とし、道路種別などから幅を推定してはならない。
- `width_rules`の値は、segmentが表す範囲の端から端までの幅[m]として扱ってよい。segment内で幅が変化する場合は、`between`の範囲ごとに扱ってよい。
- 幅データがない道路区間、幅データを解釈できない道路区間、対象範囲外の道路区間は描画しない。
- 道路幅を中心線の両側へ展開した帯状ポリゴンとして描画してよい。表示密度を抑えるため、投影後の細部を簡略化してよい。
- 該当する道路データがない場合や、対象範囲内に`width_rules`付きの区間がない場合は、道路レイヤーを空のままとしてよい。代替道路幅や仮の中心線を表示してはならない。
- 道路レイヤーは参考表示であり、地図・道路台帳・ナビゲーション用途の完全な道路表現を保証しない。
