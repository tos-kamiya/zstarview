...
- 恒星サーフェイスの基準時刻は60秒区間の中間時刻とし、区間の前半30秒を `-30`〜`0` 秒、後半30秒を `0`〜`+30` 秒として補間する。これにより、スナップショット切り替え時の位置差を抑える。
- 行列による補間量は中間時刻からの経過時間の75%を上限とする。60秒境界では小さな正方向の位置更新を残し、近似誤差による「行き過ぎて戻る」動きを抑える。
