...
- アンビエント成分は、通常時のRGB `[1, 2, 5]` から深夜2〜4時頃の `[2, 4, 10]` まで、観測地点のローカル時刻に応じて滑らかに変化させる。
...
- 人工照明が減少する深夜は、AKARI IR bands の表示強度を相対的に持ち上げてよい。
- AKARIの深夜強調は、色相やRGBチャンネルの値を変更せず、最終レイヤーopacityを時間に応じて調整する。
- 最終的なAKARI opacityは連続的に変化させ、既定設定では通常時を `0.05`、深夜2〜4時頃を `0.10` とする。
- `--akari-ir-bands-opacity` で指定する値は、深夜2〜4時頃の最大実効opacityとなる。通常時はその値の約 `1/2` まで滑らかに下がる。
- 利用者が既定値以外のopacityを指定した場合も、同じ比率で通常時の値を決める。`0` は従来どおりOFFとする。
- 深夜強調は、night light の時刻変化に対応する視覚的な相対補正であり、AKARIデータ自体の時間変化を表すものではない。
...
- 観測地点周辺の建物輪郭を、LED照明による冷たい都市光を抽象化した淡い青白色`(200, 225, 255)`で表示し、閉じた輪郭には同色の薄い面塗りを重ねてよい。
- 屋根ポリゴンの面塗りは、人工照明の一部として扱い、night light と同じ深夜係数を最終的な塗り強度へ乗算する。
- 深夜係数の適用により、屋根ポリゴンの面塗りは完全に消えてよい。面塗りを維持するための最低アルファ値を設けない。
...
- 連続光は時間経過とともに滑らかに減衰させ、深夜には街路灯を想定した約240m間隔の小さな光点を主体としてよい。
