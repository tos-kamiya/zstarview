...
- 簡易表示が有効なときは、HUD に `Simplified view [Space]` と短く表示してよい。
- 簡易表示は、次を非表示にしてよい。
  - 地点名、地点要約、時刻、ステータス情報
  - DSO
  - アステリズム
  ...
  - Earth guide
  - 副稜線
- 簡易表示でも、星、太陽、月、惑星、地平線、天の赤道、黄道、方位ラベル、天頂マーカー、天の極マーカー、および hover によるラベル表示は残してよい。
- 簡易表示中にマウスで背景を押下し、まだドラッグやサイズ変更に移行していない間は、簡易表示を一時的に反転してよい。
- その一時反転により、通常表示中は簡易表示へ、簡易表示中は通常表示へ、見た目を一時的に切り替えてよい。
- このマウス押下の一時反転は、メニュー操作、サイズ変更グリップ、またはウィンドウのドラッグには適用しなくてよい。
- ドラッグやサイズ変更が始まったら、マウス押下の一時反転は解除してよい。
- 押下継続が終わったら、表示は Space の現在状態へすぐ戻してよい。
...
- 押下中の簡易表示では、名前付きの恒星ラベルをすべて表示してよい。
