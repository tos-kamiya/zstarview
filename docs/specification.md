...
- 画面の背景部分をクリックすると、`star-focus` mode を 30 秒だけ有効にしてよい。
- `star-focus` mode は、再度クリックすると 30 秒を待たずに解除してよい。
- ウィンドウのドラッグ操作が始まった場合は、`star-focus` mode を即座に解除して通常表示へ戻してよい。
- `star-focus` mode では、なるべく星だけを見せるために次を非表示にしてよい。
  - 地点名、地点要約、時刻、ステータス情報
  - DSO
  - アステリズム
  - 雲オーバーレイ
  ...
  - 水面レイヤー
  - sky disc
- `star-focus` mode でも、星、太陽、月、惑星、地平線、天の赤道、黄道、方位ラベル、天頂マーカー、天の極マーカー、主稜線の細い線、および hover によるラベル表示は残してよい。
- `star-focus` mode の残り時間は、画面更新の idle tick で監視してよい。
