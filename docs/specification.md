...
- `--night-light-opacity` は夜間光全体の不透明度を制御してよいが、`--ridge-glow-opacity` のような別の CLI は設けなくてよい。
- ridge glow は夜間光の補助表現として内部的に扱ってよく、街灯の可視サンプルに小さな floor を足したうえで、より低い柱高で horizon 近傍へ寄せてよい。
