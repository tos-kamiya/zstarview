...
#### 6.2.1 Sky disc の太陽高度・太陽方向による色変化

sky disc の色は、太陽の高度と、各視線方向から太陽までの角距離に応じて連続的に変化する。
...
昼間の sky disc は、将来の実装で経験的な色補正の積み重ねから、大気中の単一散乱を積算するモデルへ移行する。これは現在のリリース実装とは別の置換候補であり、試作結果をもとにした仕様である。
...
- `--sky-opacity` の既定値は `0.15` とする。これにより空色を読み取りやすくする。
