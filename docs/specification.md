...
このため、同じ view center でもクライアント領域のアスペクト比が変わると、ウィンドウ内に見える天球の切り取り方は変わってよい。ただし、投影の基準角は `edge_fov_deg` であり、`content_fov_deg` は四隅の空白を減らすための余白として扱う。
...
- 帯状ポリゴンを使う場合、その幅はおおむね 2 秒程度の運動を画面上で覆う視覚表現として決めてよい。
- 帯の形状は、天球座標で細かくサンプリングした予測点列を画面へ投影した折れ線から作ってよい。これにより、急な歪みや角張りを抑えてよい。
