...
- 海岸付近の水面ドットの位置を改善するため、25m解像度の水面マスクをオプションで利用できる設計とする。
...
- 配布アセットは32×16全球グリッドの経度x列単位のZIPとし、各ZIPに該当列の全緯度タイルを格納する。タイルは既存形式の `.tif`、`.0`、`.1` を使用する。
...
- 25m水面マスクの取得は `zstarview-download-coastline --water-25m` で有効化し、`--lon-min` と `--lon-max` で必要な経度列を指定できる。`--all --water-25m` は全世界を対象とする。
