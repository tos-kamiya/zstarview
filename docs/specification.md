...
海岸線の近距離表示を改善する将来経路として、OSM Water Polygonsから生成した海岸線ベクタタイルを利用してよい。これは現時点では準備中の仕様であり、未取得の場合に現在の水面表示を失敗させてはならない。

- キャッシュの既定ルートは `CACHE_PATH/coastline/osm-water-polygons/{source-date}/schema-1/` としてよい。
- トップレベルは既存の32×16全球グリッドとし、データ量が大きいタイルだけを4×4の子タイルへ分割してよい。
- `manifest.json`には、データソース、データ日付、EPSG:4326、グリッド、分割方式、ファイルサイズ、SHA-256を記録してよい。
- 通常タイルは単一の圧縮ベクタファイル、分割タイルは子タイルのmanifestと子ファイルを保存してよい。
- 観測地点からおおむね10km以内の範囲だけを読み込み、海岸線を簡略化した境界折れ線として描画してよい。
- 分割タイルの境界をまたぐ海岸線は、隣接子タイルを同時に読み込んでも連続して見えるように扱ってよい。
- キャッシュの取得・検証・展開はバックグラウンドで行い、完全性を検証できないファイルを描画に使ってはならない。
