...
- 外部 API へ送る水面取得リクエストは、`zstarview/1.30.5 (+water-overlay)` を付けてよい。
- サービスごとに識別が必要な場合は、`zstarview/1.30.5 (+service)` 形式の接尾辞を足してよい。
...
現行のアプリケーション版は `1.30.5` であり、この節の `zstarview/1.30.5` はその現行版を表す。  
...
| --- | --- | --- |
| Overpass API (`https://overpass-api.de/api/interpreter`) | 水面レイヤーの取得 | `zstarview/1.30.5 (+water-overlay)` |
| OpenStreetMap Nominatim (`https://nominatim.openstreetmap.org/search`) | `--place` 検索 | `zstarview/1.30.5 (+nominatim)` |
| NASA Earth at Night page (`https://science.nasa.gov/earth/earth-observatory/earth-at-night/maps/`) | 夜間光データの取得元ページ参照 | `zstarview/1.30.5 (+night-lights)` |
| Overture release catalog (`https://stac.overturemaps.org/catalog.json`) | Overture 更新確認 | `zstarview/1.30.5 (+overture-release)` |
| MET Norway Geo-Satellite API (`https://api.met.no/weatherapi/geosatellite/1.4/`) | 雲オーバーレイの元画像取得 | `zstarview/1.30.5 (+geosatellite)` |
| ArcGIS FeatureServer (`https://services9.arcgis.com/RHVPKKiFTONKtxq3/arcgis/rest/services/Active_Hurricanes_v1/FeatureServer`) | 台風・サイクロン補助レイヤー | `zstarview/1.30.5 (+tropical-cyclone)` |
| ip-api.com (`http://ip-api.com/json`) | `auto` の現在地推定 | `zstarview/1.30.5 (+ip-api)` |
| OpenSky API (`https://opensky-network.org/api/states/all`) | 航空機オーバーレイ | `zstarview/1.30.5 (+opensky)` |
| CelesTrak (`https://celestrak.org/NORAD/elements/gp.php`) | 衛星 OMM 取得 | `zstarview/1.30.5 (+satellites-celestrak)` |
| JPL Horizons (`https://ssd.jpl.nasa.gov/api/horizons_lookup.api`, `https://ssd.jpl.nasa.gov/api/horizons.api`) | 衛星・小天体の位置取得 | `zstarview/1.30.5 (+satellites-horizons)` |
| WhereTheISS.at (`https://api.wheretheiss.at/v1`) | ISS TLE 取得 | `zstarview/1.30.5 (+satellites-wheretheiss)` |
| Copernicus DEM (`https://copernicus-dem-90m.s3.eu-central-1.amazonaws.com/`) | DEM タイル取得 | `zstarview/1.30.5 (+copernicus-dem)` |
| AWS S3 (`https://*.s3.amazonaws.com/`) | 衛星用の S3 バケット一覧・オブジェクト取得 | `zstarview/1.30.5 (+s3)` |
| Skyfield loader (`https://naif.jpl.nasa.gov/pub/`, `https://ssd.jpl.nasa.gov/`) | ephemeris などの Skyfield 取得 | `zstarview/1.30.5 (+skyfield-loader)` |
