...
- 昼間の基本色は白とする。薄明時は高層雲を暖色へ寄せ、低層雲は暖色を弱めた暗い灰赤色へ寄せるなど、シェルごとに異なる夕焼け補正を適用してよい。
- 太陽高度が十分高い場合と夜間は、夕焼け補正を滑らかに無効化し、既存の白〜青灰色の雲色へ戻す。
- 雲の高度情報は衛星データから厳密に観測されたものではなく、シェル分配による近似である。特定の雲の上面・下面や局所的な雲頂高度を保証しない。
- 雲シェルの色付けは空色ディスクの色をそのまま転用せず、太陽高度とシェル条件から求める。これにより、夕焼け時に雲全体が過度に赤くなることを避ける。
