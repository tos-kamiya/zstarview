...
| `--meteor-trails-opacity OPACITY` | GMNメテオ軌跡の不透明度を指定します（0.0〜1.0）。0.0ではその起動中の取得・描画・メニューからの再有効化を無効にします。 | `0.4` |
| `--meteor-trails-max-candidates N` | 地理的選別後に表示するGMNメテオ軌跡を、新しいものから最大 `N` 件に制限します。`0` で無制限にします。 | `100` |
