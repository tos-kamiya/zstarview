...
| `--night-light-opacity OPACITY` | NASA 夜間光オーバーレイのうち、街灯部分の不透明度を指定します（0.0〜1.0）。0.0 で、起動中の Black Marble のダウンロードと街灯描画を無効化します。 | `0.07` |
| `--ridge-glow-opacity OPACITY` | 地形地平線の上に描く ridge glow の不透明度を指定します（0.0〜1.0）。0.0 で、街灯レイヤーとは独立して ridge glow を無効化します。 | `0.36` |
