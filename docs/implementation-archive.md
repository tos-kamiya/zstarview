...
この文書は、`zstarview` の過去の実装履歴を残すアーカイブである。  
現在の仕様や設計の正本ではなく、過去に何をいつ実装したかを後から追えるように残すための文書である。

新しい判断や作業記録は、次を使う。

- `release-notes.md`: 利用者向けの公開履歴
- `dev-notes/session-YYYY-MM-DD.md`: 開発内部の判断や検討を含む作業記録
...
- 水面レイヤーの密度切り替え
  - 描画サーフェイスの大きさに応じて、水面サンプリングの azimuth step を `2.0° / 1.0° / 0.5°` の 3 段に切り替えるようにした。
  - 切り替え閾値は `1200px` と `2400px` に設定し、4K 級は `1.0°` に留めるようにした。
  - 仕様書、design、cache scope key、export-image 経路、関連テストを更新した。

- 台風マーカーの強度連動と表示距離拡張
  - 台風マーカーの横幅を `MAXWIND` / `maxwind_kt` 由来にし、距離が大きいほど小さくなるようにした。
  - マーカーの高さは `10km` 相当の固定基準にし、距離に応じて縮小する矩形表現へ変えた。
  - 画面に出す observer-distance cutoff を `400km` に広げた。
  - 仕様書と design の距離条件も新しい値へ合わせた。

- 台風マーカーの円すい化
  - 台風マーカーを、地面 / 水面高さの尖点と `10km` 高度の水平円盤を持つ円すい状に変更した。
  - 2 点投影に加え、円盤の周囲を地理座標でサンプリングしてから投影し、底面が地面 / 水面と平行な面として見える形へ描画した。
  - 仕様書と design も円すい表現に合わせて更新した。

- Version bump
  - `__version__` を `1.29.2` に上げた。
  - release notes の先頭を `1.29.2 - 2026-05-31` に切り替えた。
