...
def test_apply_startup_delta_t_disables_geo_clouds_for_non_present_time() -> None:
    dummy = SimpleNamespace()
    dummy.delta_t = timedelta(0)
    dummy._clouddisc = object()
    dummy._cloud_toggle_supported = True
    ...
    assert dummy._cloud_toggle_supported is False
    assert dummy.cloud_disc_alpha == 0.0
...
def test_geo_satellite_mode_is_disabled_for_non_present_time() -> None:
    dummy = SimpleNamespace(
        delta_t=timedelta(hours=8),
        viewer_data=SimpleNamespace(lat_deg=51.5, lon_deg=-0.1),
        _geo_satellite_enabled=True,
        _geosatellite_controller=object(),
        cloud_disc_alpha=0.2,
    )

    assert SkyWindow._geo_satellite_mode_active(dummy) is False
