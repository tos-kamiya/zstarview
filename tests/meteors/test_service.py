...
class _StubRepository:
    ...
    def load_window(self, window_start_utc, window_end_utc, *, now_utc=None):
        self.requested_window = (window_start_utc, window_end_utc, now_utc)
...
def test_service_loads_exact_24_hour_window_and_projects() -> None:
    ...
    assert result.window_start_utc == display_time - timedelta(hours=24)
    assert result.window_end_utc == display_time
    ...
    assert repository.requested_window == (
        display_time - timedelta(hours=24),
        display_time,
        display_time,
    )
