...
def test_reference_line_subdivision_scales_with_geometry_size() -> None:
    ...
    def fake_midpoint(start_alt: float, start_az: float, end_alt: float, end_az: float):
        _ = start_alt, start_az, end_alt, end_az
        return 45.0, 45.0

    small_painter = _PolylineCountingPainter()
    large_painter = _PolylineCountingPainter()
    original_midpoint = render_guides._interpolate_great_circle_altaz
    render_guides._interpolate_great_circle_altaz = fake_midpoint  # type: ignore[assignment]
    try:
        ...
        render_guides._interpolate_great_circle_altaz = original_midpoint  # type: ignore[assignment]

    assert max(len(polyline) for polyline in large_painter.polylines) > max(
        len(polyline) for polyline in small_painter.polylines
    )
