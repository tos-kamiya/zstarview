...
* **Space**: Cycle the display in the fixed order `normal` -> `Inverted City` -> `simplified view (no labels)` -> `simplified view (labels)` -> `visible objects only` -> `normal`. `Shift+Space` cycles in the reverse order. The startup mode (`normal`, or `Inverted City` with `--inverted-city`) determines the starting position in this cycle. When Urban Outline is disabled, `Inverted City` is skipped. The HUD shows the current display mode.
...
* **Display mode [Space]**: `Space` cycles in the fixed order normal display, `Inverted City`, simplified view without labels, simplified view with labels, and visible objects only. The startup mode (`normal`, or `Inverted City` with `--inverted-city`) determines the starting position in this cycle. If Urban Outline is disabled or locked out with opacity `0`, `Inverted City` is skipped. Simplified views hide clouds, night lights, Earth guide, secondary ridges, water, and urban outlines; the main terrain horizon stays visible in a fast-mode-like thin-line form. Visible objects only keeps visible scene layers such as the terrain horizon, water, buildings, and Earth guide, while hiding labels, asterisms, celestial guide lines, altitude rings, and never-rises; the bottom status line remains visible.
...
* **Layers**
  ...
  * **Atmosphere group**
    ...
    * **Meteor trails**: Show observed Global Meteor Network trails. `--meteor-trails-max-candidates N` limits the display to the newest `N` trails after geographic filtering; the default is `150`, and `0` removes the display limit. Within the displayed set, newer trails are brighter; the oldest trail is 50% opacity at the configured limit, and becomes brighter when fewer trails are present.
  * **Ground group**
