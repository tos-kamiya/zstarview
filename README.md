...
* **Display mode [Space]**: `Space` cycles in the fixed order normal display, `Inverted City`, simplified view without labels, simplified view with labels, and Landscape view. The startup mode (`normal`, or `Inverted City` with `--inverted-city`) determines the starting position in this cycle. If Urban Outline is disabled or locked out with opacity `0`, `Inverted City` is skipped. Simplified views hide clouds, night lights, Earth guide, secondary ridges, water, and urban outlines; the main terrain horizon stays visible in a fast-mode-like thin-line form. Landscape view keeps scene layers such as the terrain horizon, water, buildings, and Earth guide, while hiding labels, asterisms, celestial guide lines, altitude rings, and never-rises; the bottom status line remains visible.
