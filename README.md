...
- **Forecast precipitation**: the optional Open-Meteo forecast layer is disabled by default. In the `zstarview` command, enable it with a positive `--precipitation-opacity` value. The first use requires confirmation of the non-commercial Free API terms. This initial implementation is not available through `zstarview-gui` or `zstarview-export-image`.
...
| Runtime forecast precipitation data fetched from Open-Meteo | Current model-forecast precipitation sampled around the observer and modified into blue display columns; this is not observed rainfall | [Open-Meteo Weather Forecast API](https://open-meteo.com/en/docs) | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/); the Free API is limited to non-commercial use under the [Open-Meteo terms](https://open-meteo.com/en/terms) |
