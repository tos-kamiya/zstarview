...
* **Layers**
  ...
  * **Inverted City**: Display the building urban outline below the geometric horizon by reflecting its altitude around `alt=0°`. This affects building outlines, roof fills, and distant skyscraper outlines, but not road lights, terrain, water, or celestial calculations. The mode is a temporary GUI display setting and is not saved or used by `zstarview-export-image`.
