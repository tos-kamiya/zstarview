...
- **DSO 表示**: 名前付きの DSO（銀河/散開星団/球状星団）を薄い青系の領域として表示し、DSO ホバーは恒星ホバーと独立して動作します。
- **アステリウム表示**: IAU の正式な星座境界ではなく通称パターンを暗い線で常時表示し、構成星にホバーすると該当アステリウムを明るく強調してラベルを表示します。同一恒星に複数ある場合は 3 秒ごとに切り替えます。
- **太陽系天体**: 太陽・月・主要惑星に対応しています。小惑星（アステロイド）は未対応です。
- **柔軟な場所指定**: 都市名、タワー名、山名、または緯度経度を直接指定できます。
- **表示中心の調整**: `-A`（高度）/`-Z`（方位）で表示中心を調整でき、視線変更中やウィンドウサイズ変更中は一時的に簡易描画モードへ切り替えて応答性を保ちます。
- **衛星雲画像**: リアルタイムの Himawari/GOES データを縞模様（ハッチ）の重ね描きとして表示し、カメラ変更時はキャッシュ済みソースから即座に再描画できます。衛星データが部分的な場合は欠損領域を薄い黄色で示します。
- **地形地平線と地面塗り**: Copernicus DEM を使った地形地平線オーバーレイを利用でき、観測地の地形に沿った控えめな黄土色がかった地形線を表示します。幾何学的地平線または地形地平線より下は、向きの把握を助けるため地面色で塗り分けます。
- **昇らない領域**: 観測地の緯度に対して地平線の上に現れない天球領域を赤みを帯びた色で表示します。
...
[→ 例：高度 25° の東の空に表示された夏の大三角](docs/images/screenshot2.png)
...
<details>
  <summary>開発者向け補足・ライセンス・付録</summary>
