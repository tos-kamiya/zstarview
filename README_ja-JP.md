...
| :------------------------------------------ | :-------------------------------------------------------------- | :------- |
| `-h`, `--help`                              | ヘルプメッセージを表示して終了します。                                 |          |
| `--list-viewpoints {t,m}` | 同梱タワー (`t`) または山 (`m`) の主表示名を出力して終了します。各行は `t/NAME` または `m/NAME` 形式で、利用可能な場合は ASCII フォールバック名を優先します。 | |
| `--list-viewpoint-names {t,m}` | 同梱タワー (`t`) または山 (`m`) の名前を、多言語名と ASCII フォールバック名込みで一覧出力して終了します。各行は `t/NAME` または `m/NAME` 形式です。 | |
...
`--list-viewpoints` では、利用可能な場合は ASCII フォールバック名を優先表示します。
`--list-viewpoint-names` では、元の綴りと ASCII フォールバック綴りの両方を含みます。
