...
- GUI起動時ダイアログの **Celestial** タブでも、同じ **Diffuse sky source** と不透明度を選択できます。
- **アステリズム表示**: IAU の正式な星座境界ではなく、通称のパターンとしてのアステリズムを暗い線で常時表示します。アステリズムに含まれる恒星にマウスホバーすると、そのアステリズムを明るく強調してラベルを表示します。同一の恒星が複数のアステリズムに含まれる場合は 3 秒ごとに切り替えます。通常線のopacityは `--asterism-opacity`（`0.0`〜`0.5`）で指定できます。
...
- **予報降水量**: Open-Meteo の予報レイヤーは既定では無効です。現在時刻を中心にした15分区間のうち、区間の中心が現在時刻に最も近いものを選びます。雲などの観測データに基づくレイヤーとは異なり、実際の降水量と異なる場合があるモデル予報を表示します。日本などでは15分値が時間予報から補間される場合があります。`zstarview` または `zstarview-gui` の起動ダイアログで降水量の不透明度に正の値を指定すると有効になります。対話的な起動で初めて使用する際には、非商用 Free API の利用規約への同意が必要です。`zstarview-export-image` で降水量を表示するには、事前に対話的な起動で同意を保存しておく必要があります。
...
* **Space**: 表示状態を常に `通常` -> `Inverted City` -> `ラベルなし簡易表示` -> `ラベルあり簡易表示` -> `Landscape view` -> `通常` の順で切り替えます。起動時のモード（通常、または `--inverted-city` 指定時の `Inverted City`）は、この循環における開始位置を決めます。Urban Outline が無効な場合は `Inverted City` をスキップします。HUD には現在の表示状態を表示します。
* **M**: 月の 5 倍表示をトグル
...
* **表示状態 [Space]**: `Space` で、`通常`、`Inverted City`、`ラベルなし簡易表示`、`ラベルあり簡易表示`、`Landscape view`をこの順に切り替えます。起動時のモード（通常、または `--inverted-city` 指定時の `Inverted City`）は、この循環における開始位置を決めます。Urban Outline が無効または opacity `0` でロックされている場合は `Inverted City` をスキップします。簡易表示中は雲、夜間光、地球ガイド、副稜線、水面、都市アウトラインを非表示にし、主稜線は fast-mode 相当の細い線で残し、ホバーラベルは維持します。`Landscape view`では地形地平線、水面、建物、地球ガイドを表示し、ラベル、アステリズム、天文ガイド線、高度リング、never-risesを非表示にします。下部ステータス行は表示します。
...
* **Layers**
  * **Celestial グループ**
    * **Vmag limit**: 現在の恒星等級上限を読み取り専用で表示します。
    ...
    * **Diffuse sky**: 選択中のGaiaまたはAKARI背景の表示/非表示を切り替えます。
  ...
  * **Atmosphere グループ**
    ...
    * **Geo-satellite**: 実験的な MET Norway の静止気象衛星オーバーレイの表示/非表示を切り替えます。
    * **Forecast Precipitation**: 予報降水量オーバーレイの表示/非表示を切り替えます。
    * **Typhoon / Cyclone**: 現行の台風・サイクロンオーバーレイの表示/非表示を切り替えます。
    ...
    * **Meteor trails**: メテオ軌跡の重ね表示の表示/非表示を切り替えます。
  * **Ground グループ**
...
準備済みの有効なキャッシュは、`--list` ですべて一覧できます。通常表示は自治体コード、
データ年度、保存パスだけを1行ずつ表示します。`--jsonl` を指定すると、キャッシュ
メタデータを1件1行のJSONとして出力します。いずれもネットワーク接続やファイル変更は
行いません。絞り込みには `grep` などの標準的なテキスト検索ツールを使用してください。
...
カタログからファイルサイズを取得できる場合は、ダウンロード前に選択した一時
ファイルシステムの空き容量を確認します。ZIP本体、展開後のCityGMLファイル、
安全余裕を見積もり、不足していればダウンロードを開始せずに停止して
`--temp-dir` を案内します。ユーザー単位のディスククォータは個別には確認しないため、
事前確認後に空き容量が変化した場合は、処理中に `ENOSPC` として検出されます。
...
* **Gemini**、**ChatGPT**、および **Grok** に、仕様の相談、コード生成、デバッグなど、多くの助力をいただきました。
