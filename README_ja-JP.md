...
* **簡易表示 [Space] とマウス押下**: `Space` で、天体以外をなるべく消して見やすくする簡易表示を `通常` -> `ラベルなし` -> `ラベルあり` -> `通常` の 3 状態で切り替えます。背景をマウス押下すると、`通常` のときは一時的に `ラベルなし` の簡易表示へ、`ラベルなし` または `ラベルあり` のときは一時的に `通常` 表示へ戻ります。いずれもドラッグやリサイズに移るまでの一時状態で、この間は雲、夜間光、地球ガイド、副稜線、水面、都市アウトラインを非表示にし、主稜線は fast-mode 相当の細い線で残し、ホバーラベルは維持します。
