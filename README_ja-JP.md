...
* **Layers**
  ...
  * **Inverted City**: 建物の都市アウトラインを、幾何学的な地平線 `alt=0°` を基準に反転し、地平線より下へ表示します。建物の輪郭、屋根の塗り、高層建築の遠距離アウトラインが対象で、道路照明、地形、水面、天体計算は変更しません。一時的な GUI 表示設定であり、保存されず、`zstarview-export-image` では使用されません。
