...
* **Space**: 天体以外の要素をなるべく消す簡易表示をトグルします。簡易表示が有効なときは HUD に `Simplified view [Space]` を表示します。
* **M**: 月の 5 倍表示をトグル
...
* **簡易表示 [Space] とマウス押下**: `Space` で、天体以外をなるべく消して見やすくする簡易表示をトグルします。簡易表示が有効なときに背景を押下すると、ドラッグやリサイズに移るまで一時的に通常表示へ戻ります。逆に、簡易表示が無効なときの背景押下では一時的に簡易表示へ切り替わります。この間は雲、夜間光、地球ガイド、副稜線、水面、都市アウトラインを非表示にし、主稜線は fast-mode 相当の細い線で残し、ホバーラベルは維持します。
