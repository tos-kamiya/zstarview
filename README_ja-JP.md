...
* **Space**: 表示状態を `通常` -> `Inverted City` -> `ラベルなし簡易表示` -> `ラベルあり簡易表示` -> `通常` の順に切り替えます。Urban Outline が無効な場合は `Inverted City` をスキップします。HUD には現在の表示状態を表示します。
* **M**: 月の 5 倍表示をトグル
...
* **表示状態 [Space]**: `Space` で、`通常`、`Inverted City`、`ラベルなし簡易表示`、`ラベルあり簡易表示`を順に切り替えます。Urban Outline が無効または opacity `0` でロックされている場合は `Inverted City` をスキップします。簡易表示中は雲、夜間光、地球ガイド、副稜線、水面、都市アウトラインを非表示にし、主稜線は fast-mode 相当の細い線で残し、ホバーラベルは維持します。
...
* **Layers**
  ...
  * **Inverted City**: 建物の都市アウトラインを、幾何学的な地平線 `alt=0°` より下に収めて表示します。タワーなど高所から見て元々地平線より下にある建物も上空へ反転せず、地下側に留まります。建物の輪郭、屋根の塗り、高層建築の遠距離アウトラインが対象で、道路照明、地形、水面、天体計算は変更しません。一時的な GUI 表示設定であり、保存されず、`zstarview-export-image` では使用されません。
