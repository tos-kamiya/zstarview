...
- **都市アウトライン表示**: 現在地点向けにキャッシュした Overture の `building` と `building_part` データから、主要な建物屋根線を白い都市アウトラインとして表示します。`building_part` がある場合はそちらを優先します。非常に細い屋根線分は太い水平線に簡略化して表示します。
