...
- **地形地平線・地球ガイド・地面塗り**: Copernicus DEM データをダウンロードして、地形地平線オーバーレイを表示します。観測者の地点に沿った、薄い黄土色がかった地形線を表示します。地形地平線（地形地平線を表示しない場合には水平線）より下は地面色で塗り分け、さらに参考情報として、向きの把握を助けるための簡略化した大陸アウトラインを描く地球ガイドも表示します。
...
| `--terrain-horizon-opacity OPACITY` | 地形地平線ポリラインの不透明度を指定します（0.0〜1.0）。0.0 で DEM ダウンロード・地形地平線計算・描画・地球ガイドを無効化します。※4 | `0.035` |
