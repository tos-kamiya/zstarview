...
* **Space**: 表示状態を常に `通常` -> `Inverted City` -> `ラベルなし簡易表示` -> `ラベルあり簡易表示` -> `見えるものだけ` -> `通常` の順で切り替えます。起動時のモード（通常、または `--inverted-city` 指定時の `Inverted City`）は、この循環における開始位置を決めます。Urban Outline が無効な場合は `Inverted City` をスキップします。HUD には現在の表示状態を表示します。
* **M**: 月の 5 倍表示をトグル
...
* **表示状態 [Space]**: `Space` で、`通常`、`Inverted City`、`ラベルなし簡易表示`、`ラベルあり簡易表示`、`見えるものだけ`をこの順に切り替えます。起動時のモード（通常、または `--inverted-city` 指定時の `Inverted City`）は、この循環における開始位置を決めます。Urban Outline が無効または opacity `0` でロックされている場合は `Inverted City` をスキップします。簡易表示中は雲、夜間光、地球ガイド、副稜線、水面、都市アウトラインを非表示にし、主稜線は fast-mode 相当の細い線で残し、ホバーラベルは維持します。`見えるものだけ`では地形地平線、水面、建物、地球ガイドを表示し、ラベル、アステリズム、天文ガイド線、高度リング、never-risesを非表示にします。下部ステータス行は表示します。
