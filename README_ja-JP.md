...
※1 `--hours`、`--days`、`--datetime` でリアルタイムではない星空を表示した場合、雲と航空機は描画されません。人工衛星は、対応するキャッシュ済み軌道スナップショットがあれば過去表示で描画されますが、未来表示では描画されません。
...
4. 人工衛星データ
   ...
   人工衛星オーバーレイは実行時に CelesTrak から軌道要素データを取得し、fresh な current cache は最大 6 時間まで再利用します。
   古い snapshot はローカル archive へ移され、過去表示用の対応する snapshot があれば最大 3 日まで再利用されます。
