...
- **Search Objects**: 固有名星、アステリズム、地名、JPL の天体を 1 つのダイアログから検索できます。ローカル検索で見つからない場合は JPL の major body / small body にフォールバックし、`Keep marker` を有効にすると選択対象のマーカーとラベルを継続表示できます。
...
* **Search Objects...**: 固有名付き恒星、対応アステリウム、地名、JPL の天体を横断検索し、選択した対象へ移動します。ローカル検索で見つからない場合は JPL の major body / small body にフォールバックします。`Keep marker` を有効にすると、移動後もマーカーとラベルを継続表示します。
