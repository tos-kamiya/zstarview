...
| `--terrain-horizon-opacity OPACITY` | 地形地平線ポリラインの不透明度を指定します（0.0〜1.0）。0.0 で DEM ダウンロード・地形地平線計算・描画・地球ガイドを無効化します。※4 | `0.028` |
| `--earth-guide-opacity OPACITY` | 地平線下の地球ガイド線レイヤーの不透明度を指定します（0.0〜1.0）。0.0 で、その起動中の地球ガイド描画を無効化します。※4 | `0.028` |
...
3. 地形地平線
   ...
   地球ガイドだけを止めたい場合は `--earth-guide-opacity 0` を使えます。
