#!/usr/bin/env python3
"""Render water footprints from a cache JSON into a simple SVG plan view.

The script is intended for local debugging of water-overlay source data. It
reads a zstarview water-overlay cache snapshot or a compact footprint JSON file
and draws each footprint as a filled polygon in lon/lat space.

The polygons are color-coded by whether they carry explicit elevation metadata:

- `ele` or `water_level` present: highlighted fill
- no explicit height metadata: alternate fill

Each footprint is also labeled with its zero-based index so the `0/1/2` style
grouping can be inspected quickly.
"""
...
DEFAULT_OUTPUT = REPO_ROOT / "dev-samples" / "water_footprints_preview.svg"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render water footprints from a zstarview cache JSON into SVG.",
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--show-legend",
        action="store_true",
        help="Draw a small legend explaining the color coding.",
    )
    parser.add_argument(
        "--show-labels",
        action="store_true",
...
def build_svg(
    ...
    *,
    ...
    for footprint in footprints:
        fill = ELE_FILL if footprint.has_explicit_height else NO_ELE_FILL
        ...
        path_data = " ".join(part for part in (outer_path, inner_path) if part)
        body.append(
            f'<path d="{path_data}" fill="{fill}" fill-opacity="0.64" stroke="{stroke}" stroke-width="2.5" fill-rule="evenodd"/>'
        )
    ...
    if show_legend:
        ...
        legend_y = 28
        body.append(
            f'<rect x="{legend_x}" y="{legend_y - 18}" rx="8" ry="8" width="350" height="92" fill="#111827" opacity="0.82" stroke="#334155"/>'
        )
        body.append(
            f'<rect x="{legend_x + 14}" y="{legend_y - 2}" width="24" height="24" fill="{ELE_FILL}" fill-opacity="0.7" stroke="{ELE_STROKE}" stroke-width="2"/>'
        )
        body.append(
            f'<text x="{legend_x + 52}" y="{legend_y + 16}" fill="{TEXT_FILL}" font-size="16" font-family="sans-serif">ele / water_level present</text>'
        )
        body.append(
            f'<rect x="{legend_x + 14}" y="{legend_y + 30}" width="24" height="24" fill="{NO_ELE_FILL}" fill-opacity="0.7" stroke="{NO_ELE_STROKE}" stroke-width="2"/>'
        )
        body.append(
            f'<text x="{legend_x + 52}" y="{legend_y + 48}" fill="{TEXT_FILL}" font-size="16" font-family="sans-serif">no explicit height</text>'
        )
        body.append(
            f'<text x="{legend_x + 14}" y="{legend_y + 76}" fill="{TEXT_FILL}" font-size="12" font-family="sans-serif" opacity="0.75">0/1/2 style indices are shown when labels are enabled</text>'
        )
