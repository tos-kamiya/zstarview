...
def build_viewpoint(
    ...
    *,
    ...
    meta = {key: item[key] for key in meta_keys if key in item}
...
def list_viewpoint_primary_names(viewpoints: tuple[Viewpoint, ...]) -> tuple[str, ...]:
    names = {
...
def list_viewpoint_all_names(viewpoints: tuple[Viewpoint, ...]) -> tuple[str, ...]:
    ...
    return tuple(
        sorted(names_by_key.values(), key=lambda name: (normalize_viewpoint_name(name), name))
    )
