...
LAT_LON_DISPLAY_DECIMALS = 5
LAT_LON_CACHE_DECIMALS = 4


def format_lat_lon_value(value: float, *, decimals: int = LAT_LON_DISPLAY_DECIMALS) -> str:
    return f"{float(value):.{int(decimals)}f}"


def format_lat_lon_display(lat_deg: float, lon_deg: float, *, decimals: int = LAT_LON_DISPLAY_DECIMALS) -> str:
    return (
