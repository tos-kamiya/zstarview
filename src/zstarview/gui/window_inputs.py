...
class SkyWindowUserOptions:
    ...
    akari_ir_bands_opacity: float = 0.30
    ...
    meteor_trails_opacity: float = 0.5
    meteor_trails_max_candidates: int = 150
...
def prepare_window_user_options(
    *,
    ...
    return SkyWindowUserOptions(
        ...
        diffuse_sky_source=str(diffuse_sky_source).strip().lower() or "gaia",
        ...
        meteor_trails_max_candidates=max(0, int(meteor_trails_max_candidates)),
        ...
        twinkle_count=max(0, int(twinkle_count)),
