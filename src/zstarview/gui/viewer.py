    ...
    def _run(self) -> None:
        try:
            ...
            viewer_data = prepare_window_viewer_data(
                ...
                self._view_center,
                edge_fov_deg=float(getattr(self._args, "edge_fov_deg", 90.0)),
                content_fov_deg=float(getattr(self._args, "content_fov_deg", 115.0)),
    ...
    *,
    ...
    user_options = prepare_window_user_options(
        ...
        akari_ir_bands_opacity=float(
            getattr(args, "akari_ir_bands_opacity", 0.30)
        ),
        ...
        meteor_trails_opacity=float(getattr(args, "meteor_trails_opacity", 0.5)),
        meteor_trails_max_candidates=int(
            getattr(args, "meteor_trails_max_candidates", 200)
        ),
        ...
        meteor_trails_gui_allowed=(
            float(getattr(args, "meteor_trails_opacity", 0.5)) > 0.0
        ),
